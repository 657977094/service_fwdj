...
class TestDemandChangeApplyAfterRent(unittest.TestCase):

    base_url = EnvConfig.get_base_url()
    BASE_URL = f"{base_url}/crm/demandChangeApplyAfterRent"
    # BASE_URL = "http://120.52.40.45:48080/DemandChangeApplyAfterRent"   #接口地址
    headers = {
    ...
    def __init__(self, methodName='runTest'):
        ...
        self.SERVICE_CODE = "T03_TO_016"  # 服务代码
    ...
    def generate_request_xml(self,cust_company,service_code,access_token,request_time):
        ...
        return (
            ...
            '</HEAD>'
            '<BODY>'
            '<EFFECTIVE_DATE>2025-06-01</EFFECTIVE_DATE>'
            '<APPLY_BATCH>T3101184132</APPLY_BATCH>'
            '<REGION_CODE>410300</REGION_CODE>'
            '<CUST_COMPANY>1003</CUST_COMPANY>'
            '<SYS_REQUEST_TIME>2025-06-01 16:36:12</SYS_REQUEST_TIME>'
            '<Linktel></Linktel>'
            '<REQUEST_ID>1116040504116959</REQUEST_ID>'
            ...
            '<COUNTY_CODE>410381</COUNTY_CODE>'
            '<MODIFY_SOURCE>02</MODIFY_SOURCE>'
            '</BODY>'
            '</PACKET>'
        )

    def test_demandChangeApplyAfterRent_DX(self):
        #数据库响应日志
    ...
    def test_demandChangeApplyAfterRent_YD(self):
        #数据库响应日志
        # expected_response_xml = (
        #     '<?xml version="1.0" encoding="UTF-8"?>'
        #     '<PACKET>'
        ...
        # )
        try:
            request_xml = self.generate_request_xml(cust_company='1001',
                                                    service_code=self.SERVICE_CODE,
                                                    access_token=TokenGenerator.generate_access_token(self.CUST_COMPANY,
                                                                                                      self.SERVICE_CODE),
                                                    request_time=GetCurrentTime.get_current_time()).strip()
